<?php
	...
	if ($D->show_profile==1) {
		...
		$r = $this->db2->query('SELECT idlike, posts.*, username, firstname, lastname, avatar, users.iduser FROM likes, posts, users WHERE likes.iduser='.$D->u->iduser.' AND likes.idpost=posts.idpost AND posts.iduser=users.iduser ORDER BY likes.datewhen DESC LIMIT 0,'.$C->NUM_FAVORITES_PAGE);
