<?php
	...
	if ($errored == 1) {
		...
		$r = $this->db2->query('SELECT posts.*, username, firstname, lastname, avatar, users.iduser FROM posts, users WHERE posts.iduser='.$this->user->id.' AND posts.iduser=users.iduser ORDER BY posts.whendate DESC LIMIT '.$numitems.','.$C->NUM_ACTIVITIES_PAGE);
