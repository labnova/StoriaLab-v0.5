<?php 
	...
	if ($errored == 1) {
		...
		$r = $this->db2->query('SELECT idlike, posts.*, username, firstname, lastname, avatar, users.iduser FROM likes, posts, users WHERE likes.iduser='.$D->u->iduser.' AND likes.idpost=posts.idpost AND posts.iduser=users.iduser ORDER BY likes.datewhen DESC LIMIT '.$numitems.','.$itemsperpage);
